...
describe("FeeSplitterImmutable - Vault Redemption Flow", function () {
  ...
  beforeEach(async function () {
    ...
    vault = await VaultFactory.deploy(await underlying.getAddress(), "Vault Token", "VAULT");
  ...
  describe("Vault Fee Redemption Flow", function () {
    ...
    it("should handle exact asset withdrawal", async function () {
      ...
      const tx = await splitter
        .connect(ignas)
        .claimExactVaultAssets(await vault.getAddress(), assetsToWithdraw);
      const receipt = await tx.wait();
    ...
    it("should allow payees to claim their split after vault redemption", async function () {
      ...
      await splitter
        .connect(ignasSigner)
        .releaseToken(await underlying.getAddress(), IGNAS_ADDRESS);

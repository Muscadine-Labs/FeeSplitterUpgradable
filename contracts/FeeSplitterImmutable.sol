...
contract FeeSplitterImmutable is ReentrancyGuard {
    ...
    // --- views ---
    function totalShares() external view returns (uint256) { return _TOTAL_SHARES; }
    ...
    function claimVaultFeesUpToLimit(address vault) external nonReentrant 
        returns (uint256 sharesBurned, uint256 assetsOut) {
        if (vault == address(0)) revert VaultZeroAddress();
    ...
    function claimExactVaultAssets(address vault, uint256 assets) external nonReentrant 
        returns (uint256 sharesBurned, uint256 assetsOut) {
        if (vault == address(0)) revert VaultZeroAddress();
